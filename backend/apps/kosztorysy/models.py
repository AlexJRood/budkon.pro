...
class KnrPozycja(models.Model):
    ...
    katalog = models.ForeignKey(KnrKatalog, on_delete=models.CASCADE, related_name="pozycje")
    numer = models.CharField(max_length=30)          # "0201-01"
    opis = models.TextField()
    jednostka = models.CharField(max_length=20)      # "m2", "m3", "szt"
    naklad_r = models.DecimalField(max_digits=10, decimal_places=4, default=0)  # rbh/jednostkę
    naklad_m = models.DecimalField(max_digits=10, decimal_places=4, default=0)  # materiał
    naklad_s = models.DecimalField(max_digits=10, decimal_places=4, default=0)  # sprzęt rbh
...
class Kosztorys(models.Model):
    ...
    status = models.CharField(
        max_length=20,
        choices=[("roboczy", "Roboczy"), ("oferta", "Oferta"), ("zatwierdzony", "Zatwierdzony")],
        default="roboczy",
    )
...
class KosztorysdzDzial(models.Model):
    ...
    kosztorys = models.ForeignKey(Kosztorys, on_delete=models.CASCADE, related_name="dzialy")
    nazwa = models.CharField(max_length=255)
...
class KosztorysPozycja(models.Model):
    ...
    dzial = models.ForeignKey(KosztorysdzDzial, on_delete=models.CASCADE, related_name="pozycje")
    knr_pozycja = models.ForeignKey(KnrPozycja, null=True, blank=True, on_delete=models.SET_NULL)
    opis = models.TextField()
    ...
    ai_suggested_qty = models.DecimalField(max_digits=12, decimal_places=3, null=True, blank=True)
