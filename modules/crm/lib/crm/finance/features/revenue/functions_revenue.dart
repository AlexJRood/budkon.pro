...
List<PieAction> pieMenuFinancalPlansRevenue(
    WidgetRef ref, BuildContext context, planId, RevenuePlanModel plan) {
  return [
    PieAction(
...
PieAction(
  ...
  onSelect: () async {
    ...
    await ref.read(revenuePlanProvider.notifier).addPlanToRevenue(planId);
...
PieAction(
  ...
  onSelect: () async {
    final messenger = ScaffoldMessenger.of(context); // zapisujemy przed await
    ...
    builder: (context) {
      return AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ref.read(revenuePlanProvider.notifier).deleteRevenuePlan(planId);
...
void showEditPlanDialog(
    BuildContext context, WidgetRef ref, RevenuePlanModel plan) {
  showDialog(
    context: context,
    builder: (context) {
          ...
          ElevatedButton(
            onPressed: () {
              if (formKeyFunctions.currentState?.validate() ?? false) {
                ...
                final updatedPlan = RevenuePlanModel(
                  id: plan.id,
                  amount: amount,
                  currency: currency,
                  status: status,
...
void addMultiplePlansToRevenue(
    BuildContext context, WidgetRef ref, Set<int> selectedPlans) {
  for (var planId in selectedPlans) {
    // Here you would call your existing function to add plan to Revenue.
    ref.read(revenuePlanProvider.notifier).addPlanToRevenue(planId);
  }
  context.showSnackBarLikeSection('Plans added to revenue.'.tr);
}


Future<void> togglePaymentStatusForSelectedPlans(
    BuildContext context, WidgetRef ref, Set<int> selectedPlans) async {
  final messenger = ScaffoldMessenger.of(context); // zachowujemy przed await
  ...
  try {
    ...
    messenger.showSnackBar(
      SnackBar(
        content: Text('Payment status updated.'.tr),
        duration: const Duration(seconds: 2),
      ),
    );

    await ref.read(revenuePlanProvider.notifier).fetchRevenuePlans(
    ...
    messenger.showSnackBar(
      SnackBar(
        content: Text('Error updating payment status.'.tr),
        duration: const Duration(seconds: 2),
      ),
    );
  }
