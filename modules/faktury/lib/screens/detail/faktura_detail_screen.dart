...
class FakturaDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final fv = async.valueOrNull;

    final showMenu = fv != null &&
        (fv.status == StatusFaktury.szkic || fv.status == StatusFaktury.wystawiona);

    return BarManager(
      sideMenuKey: sideMenuKey,
      appModule: AppModule.budkon,
      verticalButtonsPc: showMenu
          ? PopupMenuButton<String>(
              icon: Icon(Icons.more_vert, color: theme.textColor),
              onSelected: (action) =>
                  _handleAction(context, ref, theme, action, fv!),
              itemBuilder: (_) => [
  ...
  void _handleAction(BuildContext context, WidgetRef ref, ThemeColors theme, String action, FakturaDetail fv) async {
    try {
      if (action == 'wyslij') {
